resource "aws_cloudfront_distribution" "web_cdn" {
  origin {
    domain_name              =  "web-${var.environment}.${var.zone_name}"#web-dev.imvicky.online
  ...
  aliases = ["web-${var.common_tags.Component}.${var.zone_name}"]#web-cdn.imvicky.online
...
module "records" {
  ...
  records = [
    {
      name    = "web-cdn" #web-cdn.imvicky.online
      type    = "A"
